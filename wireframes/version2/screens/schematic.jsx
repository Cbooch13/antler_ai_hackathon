// Primary Schematic Plan screen
const SchematicScreen = ({ state, setState, run, runStates }) => {
    ...
    <React.Fragment>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) 360px', gap: 16, marginTop: 14, alignItems: 'start' }}>
          ...
          <div className="plan-card">
            <div style={{ padding: '14px 16px', display: 'flex', alignItems: 'center', gap: 12, borderBottom: '1px solid var(--border)' }}>
              <div style={{ minWidth: 0 }}>
                ...
                <div style={{ color: 'var(--text-3)', fontSize: 12, marginTop: 2 }}>{lot ? lot.address : '—'} · revision attempt 3 of 3</div>
              </div>
              <span style={{ flex: 1 }} />
              {failed > 0 ? <Status kind="fail" label={`${failed} quality fails`} /> : <Status kind="passes" label="Quality OK" />}
        ...
        <div className="right-sticky" style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <Btn variant="primary" size="lg" iconRight="arrow-right" onClick={() => setState(st => ({ ...st, screen: 'packet', step: Math.max(st.step, 6) }))}>
    ...
    <Panel title="Design conversation" sub="Future tuning queue" flush>
      <div style={{ padding: 12, display: 'flex', flexDirection: 'column', gap: 8 }}>
        ...
        <div className="convo-input">
          <input className="input" placeholder="Ask the schematic agent…" value={val} onChange={e => setVal(e.target.value)} onKeyDown={e => e.key === 'Enter' && send()} />
...
// Schematic floor plan SVG — conceptual, drawn from room schedule
const FloorPlanSVG = () => {
  // Coordinates (in plan units) chosen to look like a duplex shotgun
  // Footprint 100 x 67 plan units
  const W = 100, H = 67;
  const rooms = [
  ...
  // Add label-overflow guard via clip
  return (
...
window.SchematicScreen = SchematicScreen;
