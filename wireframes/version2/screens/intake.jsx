// Project Workspace / Intake screen
const IntakeScreen = ({ state, setState, run, runStates }) => {
  const intake = state.intake;
  const updateIntake = (key, value) => setState(s => ({ ...s, intake: { ...s.intake, [key]: value }}));
  const removeStyle = (style) => updateIntake('stylePreferences', intake.stylePreferences.filter(s => s !== style));
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div className="field">
              ...
              <div className="segment" role="radiogroup">
                {['Low', 'Medium', 'High'].map(t => (
                  <button key={t} role="radio" aria-checked={intake.riskTolerance === t} className={intake.riskTolerance === t ? 'on' : ''} onClick={() => updateIntake('riskTolerance', t)}>{t}</button>
                ))}
              </div>
            </div>
            <div className="field">
            ...
            <div className="field field-full">
              ...
              <div className="chip-row">
                {intake.stylePreferences.map(s => (
                  <span key={s} className="chip removable">{s}<button onClick={() => removeStyle(s)} aria-label={`Remove ${s}`}>×</button></span>
                ))}
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <Btn size="lg" variant="primary" loading={findLotsLoading} iconRight={findLotsLoading ? null : 'arrow-right'} onClick={() => run('findLots')} disabled={!validated}>
              Find prototype lots
            </Btn>
          </div>
        </div>
            ...
            {validateLoading ? (
              ...
              <React.Fragment>
                ...
                <div>
                  ...
                  {ABF_DATA.validatedContract.missingFields.length === 0
                    ? <Status kind="passes" label="None" />
                    : <Status kind="warning" label={`${ABF_DATA.validatedContract.missingFields.length} missing`} />}
                </div>
                <div>
                  <div className="label" style={{ marginBottom: 4 }}>Assumptions</div>
                  <ul className="lot-reasons">
                    {ABF_DATA.validatedContract.assumptions.map((a, i) => <li key={i}>{a}</li>)}
                  </ul>
                </div>
                {ABF_DATA.validatedContract.warnings.length > 0 && (
                  <div>
                    <div className="label" style={{ marginBottom: 4 }}>Warnings</div>
                    {ABF_DATA.validatedContract.warnings.map((w, i) => (
