// Mobile view — same workflow rendered for narrow viewport in iOS frame
const MobileView = ({ state, setState, run, runStates, onClose }) => {
  const screen = state.screen;
  return (
    <div className="m-screen">
      <div className="m-topbar">
        <div className="m-row1">
          <button className="btn btn-sm btn-ghost btn-icon" onClick={onClose} aria-label="Close mobile preview"><Icon name="back" size={14}/></button>
          <div style={{ minWidth: 0 }}>
            <div className="m-title">Austin Build Feasibility AI</div>
            <div style={{ fontSize: 10.5, color: 'var(--text-3)', fontFamily: 'var(--font-mono)' }}>{ABF_DATA.project.name}</div>
          </div>
          <span style={{ flex: 1 }} />
      ...
      <div className="m-stepper">
        {STEPS.map((s, i) => {
          ...
          return (
            <button key={s.key} className={'m-step ' + status} onClick={() => i + 1 <= state.step && setState(st => ({ ...st, screen: s.key }))}>
      ...
      <div className="m-content">
        {screen === 'intake' && <MobileIntake state={state} setState={setState} run={run} runStates={runStates}/>}
        {screen === 'lots' && <MobileLots state={state} setState={setState} runStates={runStates}/>}
        {screen === 'lot-context' && <MobileLotContext state={state} run={run} runStates={runStates}/>}
        {screen === 'compliance' && <MobileCompliance run={run} runStates={runStates}/>}
        {screen === 'schematic' && <MobileSchematic setState={setState}/>}
        {screen === 'packet' && <MobilePacket />}
      </div>

      <div className="m-disclaimer"><span className="d" style={{ width: 5, height: 5, background: 'var(--warn)', borderRadius: '50%' }}/>Advisory feasibility — not official Austin approval.</div>
    </div>
  );
};

const MobileIntake = ({ state, setState, run, runStates }) => {
  const i = state.intake;
  const update = (k, v) => setState(s => ({ ...s, intake: { ...s.intake, [k]: v }}));
  return (
    <React.Fragment>
      <div className="m-card">
        <div className="m-card-h"><strong>Project intake</strong><Status kind={state.validated ? 'passes' : 'unknown'} label={state.validated ? 'Validated' : 'Pending'}/></div>
        <div className="m-card-b" style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          ...
          <Btn size="sm" loading={runStates.validate === 'loading'} onClick={() => run('validate')}>Validate spec</Btn>
        </div>
      </div>
      ...
      <div className="m-card">
        ...
        <div className="m-card-b" style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          ...
          <Btn size="sm" icon="refresh" loading={runStates.normalize === 'loading'} onClick={() => run('normalize')}>Normalize</Btn>
        </div>
      </div>
      ...
      <div className="m-card">
        ...
        <div className="m-card-b" style={{ padding: 0 }}>
          <JsonView data={ABF_DATA.validatedContract.spec}/>
        </div>
      </div>

      <Btn size="lg" variant="primary" iconRight="arrow-right" loading={runStates.findLots === 'loading'} disabled={!state.validated} onClick={() => run('findLots')}>
        Find prototype lots
      </Btn>
    </React.Fragment>
  );
...
const MobileLots = ({ state, setState, runStates }) => (
  <React.Fragment>
    ...
    <div style={{ marginLeft: -12, marginRight: -12, marginTop: 4 }}>
      <div className="m-swipe-row">
        {ABF_DATA.lots.map((l, i) => {
          const sel = state.selectedLotId === l.id;
          return (
            <div key={l.id} className="m-lot-card" onClick={() => setState(s => ({ ...s, selectedLotId: l.id, step: Math.max(s.step, 2) }))} style={{ borderColor: sel ? 'var(--teal)' : undefined, boxShadow: sel ? '0 0 0 1px var(--teal) inset' : undefined }}>
...
const MobileLotContext = ({ state, run, runStates }) => {
  ...
  return (
    <React.Fragment>
      ...
      <Btn size="lg" variant="primary" loading={runStates.feasibility === 'loading'} iconRight="arrow-right" onClick={() => run('feasibility')}>Run feasibility check</Btn>
    </React.Fragment>
  );
};

const MobileCompliance = ({ run, runStates }) => {
    ...
    <React.Fragment>
      ...
      <div className="m-tbl-wrap">
        <table className="tbl">
          ...
          <tbody>
            {c.rows.map((r, i) => (
              <tr key={i}><td className="text-2">{r.category}</td><td style={{ fontWeight: 500 }}>{r.metric}</td><td className="measure">{r.value}</td><td><Status kind={r.status}/></td><td><span className="badge neutral"><span className="d"/>{r.confidence}</span></td><td className="text-2">{r.notes}</td></tr>
            ))}
          </tbody>
        </table>
      </div>
      ...
      <Btn size="lg" variant="primary" loading={runStates.plan === 'loading'} iconRight="arrow-right" onClick={() => run('plan')}>Generate schematic plan</Btn>
    </React.Fragment>
  );
};
const FindingColMobile = ({ kind, title, items }) => (
  <div className="finding-col">
    <div className="finding-col-h"><span className="t">{title}</span><Status kind={kind} label={String(items.length)}/></div>
    {items.length === 0 ? <div style={{ padding: 10, textAlign: 'center', color: 'var(--text-3)', fontSize: 11 }}>none</div> : items.slice(0, 2).map((f, i) => <div key={i} className="finding"><div className="finding-t">{f.title}</div></div>)}
  </div>
);

const MobileSchematic = ({ setState }) => {
  const s = ABF_DATA.schematic;
  return (
    <React.Fragment>
      <div className="m-card">
        <div className="m-card-h"><strong>{s.name}</strong><Status kind="fail" label={`Q ${s.qualityScore}/100`}/></div>
        <div style={{ padding: 10 }}><FloorPlanSVG/></div>
        <div className="plan-stats" style={{ gridTemplateColumns: '1fr 1fr', borderTop: '1px solid var(--border)' }}>
      ...
      <div className="m-card">
        ...
        <div className="m-tbl-wrap" style={{ border: 0, borderRadius: 0 }}>
          <table className="tbl">
            ...
            <tbody>{s.rooms.map((r,i) => <tr key={i}><td style={{ fontWeight: 500 }}>{r.name}</td><td className="measure">{r.sqft}</td><td className="measure">{r.dim}</td><td className="measure">{r.level}</td></tr>)}</tbody>
          </table>
        </div>
      </div>
      <div className="m-card">
        <div className="m-card-h"><strong>Quality checks</strong></div>
        <div className="qc-list">{s.qualityChecks.map((q,i) => <div key={i} className={'qc-row' + (q.status === 'fail' ? ' fail' : '')}><div className={'qc-mark ' + q.status}>{q.status === 'pass' ? '✓' : '!'}</div><div><div className="qc-name">{q.name}</div><div className="qc-body">{q.body}</div></div><div/></div>)}</div>
      </div>
      <Btn size="lg" variant="primary" iconRight="arrow-right" onClick={() => setState(st => ({ ...st, screen: 'packet', step: Math.max(st.step, 6) }))}>Assemble review packet</Btn>
    </React.Fragment>
  );
};
        ...
        <div key={g.role} className="m-card">
          ...
          <div className="m-card-b">
            ...
            <div className="doc-list">
              {g.documents.map((d,i) => (
                <div key={i} className="doc-row"><div className={'doc-status-icon ' + d.status}>{d.status === 'ready' ? '✓' : d.status === 'draft' ? '◐' : '·'}</div><div className="doc-name">{d.name}{d.required && <span className="req">*</span>}</div><div/><Status kind={STATUS_BADGE[d.status].kind} label={STATUS_BADGE[d.status].label}/></div>
              ))}
            </div>
