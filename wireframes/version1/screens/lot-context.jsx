// Lot Context screen
const LotContextScreen = ({ state, setState, run, runStates }) => {
    ...
    <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) 360px', gap: 16 }}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        <Panel
          ...
          flush
        >
          <div className="map-placeholder" style={{ borderRadius: 0, borderLeft: 0, borderRight: 0, borderTop: 0 }}>
            <div className="map-overlay">
              <div><strong>{lot.address}</strong></div>
              <div>{lot.coords.lat.toFixed(3)}, {lot.coords.lng.toFixed(3)}</div>
            </div>
            <div className="map-pin" />
            <div className="map-attribution">map placeholder · Mapbox token not configured</div>
          </div>
          <div style={{ padding: 14, display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <span style={{ flex: 1 }} />
            <span className="kbd">aerial preview disabled</span>
          </div>
        </Panel>
        ...
        <Panel title="Source limitations & joins" flush>
          <div style={{ padding: 14, display: 'flex', flexDirection: 'column', gap: 8 }}>
            ...
            <Banner kind="neutral" title="Aerial imagery unavailable">
              Mapbox token is not configured. Aerial image URL is unavailable in this build.
            </Banner>
          </div>
        </Panel>
      </div>
...
window.LotContextScreen = LotContextScreen;
