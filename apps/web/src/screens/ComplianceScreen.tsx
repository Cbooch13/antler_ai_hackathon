...
import { compliance, lots } from '../data/mock';
import type { ComplianceFindingItem } from '../types';

function FindingCol({ kind, title, items }: { kind: string; title: string; items: ComplianceFindingItem[] }) {
  return (
...
export function ComplianceScreen() {
  ...
  const groups = c.rows.reduce<Record<string, typeof c.rows>>((acc, r) => {
    ...
    <>
      ...
      <div className="panel" style={{ overflow: 'hidden' }}>
        <table className="tbl">
          ...
          <tbody>
            {Object.entries(groups).flatMap(([, rows], gi) =>
              rows.map((r, ri) => (
                <tr key={`${gi}-${ri}`} className={ri === 0 && gi > 0 ? 'sep' : ''}>
                  <td className="col-cat">{ri === 0 ? r.category : ''}</td>
      ...
      <div style={{ marginTop: 18, display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
        ...
        <Btn
          variant="primary"
          size="lg"
          iconRight={planLoading ? null : 'arrow-right'}
          loading={planLoading}
          onClick={() => run('plan')}
        >
