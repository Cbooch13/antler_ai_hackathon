...
import { Btn, Status, Banner, Skel } from '../components';
import { fmt } from '../components';
import { lots } from '../data/mock';
import { PHOTOS } from '../photos';

export function LotsScreen() {
  const { state, setState, runStates } = useWorkflow();
  const navigate = useNavigate();
  const photos = PHOTOS.lots;
  const isLoading = runStates.findLots === 'loading';

  const select = (id: string) => {
    setState(s => ({ ...s, selectedLotId: id, step: Math.max(s.step, 2) }));
    navigate('/workspace/lot-context');
  };
      ...
      <div className="card-list">
        {[0, 1, 2, 3].map(i => (
          <div key={i} className="lot-card" style={{ cursor: 'default' }}>
            <div className="lot-thumb" style={{ background: 'var(--bg-warm)' }}>
              <Skel h="100%" w="100%" style={{ borderRadius: 0 }} />
            </div>
        ...
        {lots.map((l, i) => {
          ...
          return (
            <div
              key={l.id}
              data-lot={l.id}
              className={'lot-card' + (selected ? ' selected' : '')}
              onClick={() => select(l.id)}
            >
              ...
              <div className="lot-card-body">
                ...
                <div className="lot-stats">
                  ...
                  <div className="ls"><span className="ls-l">Building</span><span className="ls-v">{l.buildingSqft ? <><span className="mono">{fmt.num(l.buildingSqft)}</span> sqft</> : <span className="muted">—</span>}</span></div>
