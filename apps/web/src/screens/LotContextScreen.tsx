...
import { Btn, Panel, Banner, LockedState } from '../components';
  ...
  if (!lot) {
    return (
      <LockedState
        ...
        body="Pick a candidate from Lot Discovery to load its parcel context, sources, and warnings."
        action={<Btn size="sm" onClick={() => navigate('/workspace/lots')}>Go to Lot Discovery</Btn>}
      />
    );
  }

  const photo = PHOTOS.aerials[lot.id];
    ...
    <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) 380px', gap: 24 }}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        >
          <div
            ...
            style={{ borderRadius: 0, border: 0, height: 360, backgroundImage: `url(${photo})` }}
          >
          ...
          <div style={{ padding: 16, display: 'flex', alignItems: 'center', gap: 8, borderTop: '1px solid var(--border)' }}>
            <a className="btn btn-sm" href={mapUrl} target="_blank" rel="noreferrer">
              <Icon name="map" size={13} />&nbsp;Open in Maps
            </a>
            <a
              className="btn btn-sm"
              href={`https://www.google.com/maps?layer=c&cbll=${lot.coords.lat},${lot.coords.lng}`}
              target="_blank"
              rel="noreferrer"
            >Street view</a>
            <span style={{ flex: 1 }} />
      ...
      <div className="right-sticky" style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        <Btn
          size="lg"
          variant="primary"
          iconRight={feasibilityLoading ? null : 'arrow-right'}
          loading={feasibilityLoading}
          onClick={() => run('feasibility')}
        >
