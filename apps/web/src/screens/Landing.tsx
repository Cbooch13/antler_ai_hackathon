...
export function Landing() {
  ...
  const onEnter = () => navigate('/workspace/intake');

  return (
    <div className="landing">
      <header
        className="topbar"
        style={{ position: 'absolute', top: 0, left: 0, right: 0, zIndex: 10, background: 'transparent', borderBottom: 0, color: '#fff' }}
      >
        ...
        <div className="project-meta" style={{ color: 'rgba(255,255,255,0.7)' }}>
          <span>Workspace</span>
          <span className="dot" style={{ background: 'rgba(255,255,255,0.4)' }} />
          <span style={{ fontFamily: 'var(--font-mono)', fontSize: 11 }}>v0.7.4</span>
        </div>
        <span className="advisory-badge" style={{ background: 'rgba(255,255,255,0.12)', borderColor: 'rgba(255,255,255,0.3)', color: '#fff' }}>
          <span className="pulse" />Advisory feasibility · not City approval
        </span>
        <div className="topbar-spacer" />
        <div className="topbar-actions">
          <button className="hero-btn ghost" style={{ height: 34, padding: '0 14px', fontSize: 12.5 }} onClick={onEnter}>
            Open workspace<Icon name="arrow-right" size={13} />
          </button>
        </div>
      </header>
          ...
          <div className="intake-launch-left">
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                <button className="hero-btn primary" style={{ background: 'var(--text)', color: '#fff', borderColor: 'var(--text)' }} onClick={onEnter}>
                  Open workspace<Icon name="arrow-right" size={14} />
                </button>
                <button className="hero-btn ghost" style={{ borderColor: 'var(--border-strong)', color: 'var(--text)' }} onClick={onEnter}>
                  Continue PRJ-0247
                </button>
              </div>
