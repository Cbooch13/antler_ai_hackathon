...
Stage 6G establishes the system foundation, the first user-facing intake path, official Austin public-data ingestion, MVP lot discovery, selected-lot context, advisory compliance feasibility, generative schematic-agent planning, conceptual architectural floor plans, deterministic floor-plan quality validation, and OpenAI revision-loop feedback:
...
Stage 6E schematic options are generative planning outputs generated by the Schematic Design Agent from the selected lot, validated build spec, property context, and compliance findings. When `LLM_SCHEMATIC_ENABLED=true` and `OPENAI_API_KEY` is configured, the Design Agent uses OpenAI structured outputs; otherwise it uses the local generator fallback. The service returns:
...
- OpenAI revision-loop assumptions showing which attempt was accepted or whether the best available advisory result was returned
