...
Stage 6K schematic options are generative planning outputs generated by the Schematic Design Agent from the selected lot, validated build spec, property context, compliance findings, and retrieved plan exemplars. When `LLM_SCHEMATIC_ENABLED=true` and `OPENAI_API_KEY` is configured, the Design Agent uses OpenAI structured outputs; otherwise it uses the local generator fallback. The service returns:
