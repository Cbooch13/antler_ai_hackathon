...
Stage 6L establishes the system foundation, the first user-facing intake path, official Austin public-data ingestion, MVP lot discovery, selected-lot context, advisory compliance feasibility, generative schematic-agent planning, conceptual architectural floor plans, deterministic floor-plan quality validation, OpenAI revision-loop feedback, structured geometry refinement, architectural plan rendering, adjacency/circulation validation, exemplar-guided planning, best-of-7 candidate selection, research-reference prompt guidance, and stricter plan-realism constraints:
...
- Retrieved plan exemplars that provide room-area ratios, adjacency precedents, and planning notes to the LLM prompt and local generator.
- Local schematic generation that creates seven exemplar-guided candidates, scores them with deterministic quality and exemplar-fit checks, and returns only the best primary plan.
- Research-reference prompt payloads for ResPlan, RPLAN, Tell2Design, Graph2Plan, HouseLLM, DStruct2Design, and ZURU/AWS as method guidance, not public-model dependencies.
- Explicit prompt policy that asks the model to use references as structured planning guidance without returning hidden chain-of-thought.
- Stricter realism checks for bedroom hall/entry access, synthetic path repairs, circulation efficiency, room-area balance, and wet-core grouping.
...
Stage 6L schematic options are generative planning outputs generated by the Schematic Design Agent from the selected lot, validated build spec, property context, compliance findings, retrieved plan exemplars, and research-method references. When `LLM_SCHEMATIC_ENABLED=true` and `OPENAI_API_KEY` is configured, the Design Agent uses OpenAI structured outputs; otherwise it uses the local generator fallback. The service returns:
...
- retrieved-exemplar assumptions showing which precedent pattern guided the selected plan
- local best-of-7 assumptions showing that seven candidates were generated/scored before returning one plan
- OpenAI best-of/revision assumptions showing which generation met the `>= 85` quality threshold or whether the best available advisory result was returned
- stricter quality checks for direct bedroom access, no synthetic path repair, circulation efficiency, room-area balance, and wet-core grouping
- carried-forward compliance findings
- warnings that outputs are conceptual and require professional review

Floor plans are generated concept plans for early product review. The LLM or local generator must reconcile room square footage to the requested building square footage and should follow basic architectural planning practices such as retrieved-precedent area ratios, public/private zoning, grouped wet rooms, circulation control, direct bedroom hall access, exterior openings, and separated second-unit access where relevant. They are not measured architectural drawings and do not validate egress, MEP, structural, accessibility, fire, or permit requirements.
