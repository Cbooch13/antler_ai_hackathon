...
Stage 6D establishes the system foundation, the first user-facing intake path, official Austin public-data ingestion, MVP lot discovery, selected-lot context, advisory compliance feasibility, generative schematic-agent planning, and conceptual architectural floor plans:
...
Stage 6D schematic options are generative planning outputs generated by the Schematic Design Agent from the selected lot, validated build spec, and compliance findings. The service returns:
